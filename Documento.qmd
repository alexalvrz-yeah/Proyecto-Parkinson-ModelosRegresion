...
train  <- subset(parkinson_df, subject %in% lista[[1]][1:30]) # 30 sujetos
test  <- subset(parkinson_df, subject %in% lista[[1]][31:36]) # 6
val  <- subset(parkinson_df, subject %in% lista[[1]][37:42]) # 6
...
```{r}
train %>% count(subject, sex, age, sort=TRUE)
```
...
`test_time` tiene valores negativos. Teniendo en cuenta lo que
significa, tendríamos que quitarlos (no tiene sentido que haya)
...
Algo muy a tener en cuenta cuando exploremos estas variables es que el
...
ggplot(df, aes(sexo, y, fill=sexo)) + labs(sexo = NULL, y = NULL) + geom_bar(stat = "identity") + scale_fill_manual(values=c("pink","blue")) + ggtitle("Sexo de los pacientes") 
```
...
Luego de hacer algo del EDA empezar esto. Eso sí, hay que hacer más
cosas que faltan por añadir después abajo.
