...
Veamos la correlación entre ambos valores, quitando en el segundo caso
al atípico 25 para ver si es tan influyente:
...
```{r}
ggcorr(data = jitter_escalado, method = c("everything", "pearson"), label = TRUE, label_round = 3) # pearson es el supuesto default, ahora miro si hay alguno mejor
```
...
Se ve un 1 entre `rap` y `ddp`, una de las dos ha de ser borrada
después.
...
```{r}
ggcorr(data = shimmer_escalado, method = c("everything", "pearson"), label = TRUE, label_round = 3)
```

Hay números muy cercanos a 1 en todos, el más diferente es
`shimmer_apq10`.

De nuevo, hay otro 1 **por hacer**.

Como último experimento, voy a juntar tanto `jitter*` como `shimmer*`
por investigar:

```{r}
ambos_escalados <- scale(train[ ,7:17])
ggcorr(data = ambos_escalados, method = c("everything", "pearson"), label = TRUE, label_round = 1)
```
