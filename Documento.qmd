...
descr(train_descr, stats = c("mean", "sd", "skewness", "kurtosis", "max", "min"), transpose = TRUE, headings= FALSE)
```
...
duplicadas. Veremos si esto es cierto.
...
tiempo en el cual tienen el `motor_UPDRS` más alto.
...
Después de todas estas comprobaciones, podemos confirmar que escogeremos
`motor_UPDRS` como variable a predecir, ya que `total_UPDRS` es menos
predecible gracias a la posibilidad de que existan más sujetos como el
25.
...
para denotar normalidad) puedo ver que está cerca de ser normal. Sin
embargo, al ser el n tan grande (4170 observaciones), genera un poco de
duda ya que no está más cerca del 0.
...
ggplot(mapping = aes(logshimmer)) + geom_histogram() + labs(title = "Logaritmo de shimmer (cola derecha demasiado fuerte)")
# voy a hacer un test más
...
Como último experimento, vamos a juntar tanto `jitter*` como `shimmer*`
por investigar:
...
La distribución de esta variable tiene la forma de una normal con una
gran cola izquierda.
