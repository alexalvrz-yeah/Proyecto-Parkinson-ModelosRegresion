...
### Regresión lineal simple con transformación antes
...
Este modelo, como era de esperar, no tiene buena pinta. Vamos a empezar
con la selección de variables, usando el criterio de informacion de
Akaike (AIC) de forma stepwise. Como AIC está orientado a predicción, es
mejor que hacerlo con BIC para nosotros.
...
Vemos que el RMSE emperora pero vemos una mejora de multicolinealidad, y
mejora el MAE y el MAPE. Pero aún así, en tema de supuestos seguimos con
un problema pese a las mejoras en métricas. Es decir, sigue siendo
inválido por este motivo. Podríamos haber usado regularización para
selección de variables igual para reducir el VIF, pero vemos que tenemos
problemas de relaciones mas que de correlacción entre predictores.
...
Bueno, consigue mejorar un poco el MAPE.
...
mape_gamma <- mean(abs((probar_sucio$motor_UPDRS - pred_gamma) / test_selection$motor_UPDRS)) * 100
...
Vemos que disminuye el RMSE, lo que indica que sus errores ya no son tan
extremos, pero vemos un aumento en el error porcentual, lo que nos dice
que los errores en promedio sí son mas grandes, aún sin llegar a grandes
extremos lo que nos indica la bajada del RMSE. Si lo hubiésemos visto,
aquí aplicaríamos suavizado.

------------------------------------------------------------------------

## Modelos de Regresión Generalizada

Como no tenemos datos de conteo, no vamos a aplicar Poisson. Lo que
vamos a aplicar, eso sí, es la regresión logística. Tenemos que
binarizar el target con algún criterio. Después de investigar un poco
por el internet, y teniendo en cuenta que no tenemos acceso a un
experto, no existe un punto exacto en el valor del UPDRS en el cual se
pueda decir que la gravedad del Parkinson es mucho mayor, y tampoco
podemos saber si hay un punto que interese a los científicos. Así que
nuestro criterio para binarizar será uno simple: 50% más bajos y 50% más
altos.
...
## Usando PCA como último recurso
...
muestras de `validation`. El modelo que usaremos, ya que estamos
tratando de predecir lo mejor posible, es aquel que tiene los mejores
AIC, R cuadrado ajustado, RMSE y MAPE. (medidas que hemos sacado a lo
largo de este trabajo).

Ese mejor modelo es (), que tiene los parámetros (). Vamos a probar el
modelo con val_selection, y veamos qué valores nos da:
...
Y, si los hubíesemos visto, habríamos usado un modelo mixto (que sí
habría dado un buen resultado). Resulta que los modelos mixtos son
parecidos a lo de "sacar una media de los grupos".
...
rmse: raíz cuadrada del mse, mae: error medio en valor absoluto y mape:
porcentaje de lo anterior
