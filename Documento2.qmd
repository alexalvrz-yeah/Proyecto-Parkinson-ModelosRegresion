...
Vemos un RMSE y un MAPE más bajo que en los anteriores.
...
tratando de predecir lo mejor posible, es aquel que tiene los mejores R
cuadrado ajustado, RMSE o MAPE. (medidas que hemos sacado a lo largo de
este trabajo, algunas no pueden ser obtenidas de algunos modelos).

Ese mejor modelo es (), que tiene como parámetros (). Vamos a probar el
...
consistente las mediciones, todos los días en vez de aproximadamente más
de una vez cada dos días de forma irregular).

Y, si los hubíesemos visto, habríamos utilizado un modelo mixto (que
