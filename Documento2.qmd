...
abajo, en la sección de [PCA o Selección de Variables]. `total_UPDRS` es
excluida porque no la tendremos a la hora de predecir. `sex` y `age` son
excluidas porque causarían sobreajuste (`age` predice perfectamente al
sujeto de 36 años porque sabe quién es con la edad, sería como predecir
con `subject`, `sex` haría algo similar pero entre los dos sexos).
...
- `jitter*` y `shimmer*` forman dos grupos con correlación alta y
  ...
  (dos con correlación 1) y `jitter_abs` es distinta a todas.
...
hacer modelo con dfa y motor_updrs aquí.
...
estadístico F da 50, eso sí.

hagámoslo con otro candidato, hnr

```{r}
lmhnr <- lm(motor_UPDRS~hnr, data = train)
summary(lmhnr)
ggplot(train) + geom_point(aes(x = hnr, y =motor_UPDRS)) + geom_abline(intercept = 27.94262, slope = -0.30651, color = "red", lwd = 2)
```
...
```{r}
lmsh11 <- lm(motor_UPDRS~shimmer_apq11, data = train)
summary(lmsh11)
ggplot(train) + geom_point(aes(x = shimmer_apq11, y =motor_UPDRS)) + geom_abline(intercept = 19.7994, slope = 54.0666, color = "red", lwd = 2)
```

le haría falta una transformación

sería interesante hacerlo también con test_time para ver si hace un
modelo que no sea extremadamente lamentable y mostrar que esta variable
sería útil si los pacientes no se medicasen tampoco(?).

```{r}
lmtt <- lm(motor_UPDRS~test_time, data = train)
summary(lmtt)
ggplot(train) + geom_point(aes(x = test_time, y =motor_UPDRS)) + geom_abline(intercept = 20.290554, slope = 0.010494, color = "red", lwd = 2) 
```

nada, esta variable no serviría incluso si pudiesemos usarla.
...
forma una nube, no parece verse nada
...
cambia mucho todo pero no deja de ser una nube rara
...
la nube que forma es muy similar a la de la anterior new
...
como encima sabemos, por mucho que no se pueda ver, que los residuos
están correlacionados, podemos concluir que este modelo no cumple con
NINGUNO!!! de los supuestos. waow.
